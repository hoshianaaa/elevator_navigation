...
SearchPanel::SearchPanel()
{
	...
	scan_sub_ = n_.subscribe("scan", 1, &SearchPanel::scanCallback, this);
...
void SearchPanel::scanCallback(const sensor_msgs::LaserScan::ConstPtr& msg){
...
bool SearchPanel::rotate(double angle)
{
...
bool SearchPanel::darknet_wait(long wait_time){
	darknet_lock_ = 1;
...
bool SearchPanel::go_panel(double stop_distance){
	ros::Rate loop_rate(freq_);
	while(1){
...
void SearchPanel::run(bool inverse)
{
	int counter = 0;
	double k = -0.003;
	while(1){
